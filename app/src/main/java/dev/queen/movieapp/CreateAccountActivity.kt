...
class CreateAccountActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        createAccountInputArray = arrayOf(bindingCreate.etEmail, bindingCreate.etPassword, bindingCreate.etConfirmPassword)
        setOnclicks()
    }
    ...
    private fun register() {
        ...
        if (identicalPassword()){
            ...
            var email = bindingCreate.etEmail.toString().trim()
            var psswd = bindingCreate.etPassword.toString().trim()

            //create a user
            firebaseAuth.createUserWithEmailAndPassword(email, psswd).addOnCompleteListener { task ->
    ...
    private fun sendEmailVerification() {
        var emailInfo = bindingCreate.etEmail.text.toString()
        firebaseUser?.let {
            it.sendEmailVerification().addOnCompleteListener { task ->
                if (task.isSuccessful) {
                    Toast.makeText(this, "email sent to $emailInfo", Toast.LENGTH_LONG).show()
                }
            }
        }
    }
